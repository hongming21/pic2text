import sys, datetime, time,json,string,os
import numpy as np
import torch
import torchvision
...
class Image_text_logger(Callback):
    ...
    def log_local(self, split, data,
        ...
        root = os.path.join(self.save_dir,'result',split)
        for k in data:
            if k=="input_img":
...
if __name__ == "__main__":
    ...
    # 初始化回调
    img_logger_callback = Image_text_logger(save_dir=logdir, train_batch_frequency=100, val_batch_frequency=50, max_log=4, log_on_batch_idx=True)
    ...
    cli = LightningCLI(
        trainer_defaults={
            'logger': TensorBoardLogger(save_dir='logs', name=now, version=0),
